        ...
        <BoxContent title={es.technologies.title} light={true}>
          <div className="flex flex-col lg:flex-row gap-2">
            <div className="flex justify-center flex-col flex-1 gap-3">
              <h2 className="font-bold text-5xl leading-normal text-dark-blue-100 first-line:text-white">
                {es.technologies.action.title}
              </h2>
              <p className="text-lg">{es.technologies.action.description}</p>
              <ul className="grid grid-cols-3 xl:grid-cols-5 2xl:grid-cols-7 text-dark-gray mt-10 border-t border-l border-dark-blue-200">
                {es.technologies.tech?.map((tech, index) => (
                  <li
                    className="border-dark-blue-200 border p-3 grid place-items-center border-l-0 border-t-0"
                    key={index}
                  >
                    <tech.icon className="size-[72px]" />
                    <span className="mt-1 font-thin">{tech.text}</span>
                  </li>
                ))}
              </ul>
              <InternalLink
        ...
        >
          <div className="w-full max-w-[960px] 2xl:max-w-max">
            ...
            <div className="mt-8 max-w-[768px] mx-auto text-center">
              <h2 className="font-bold text-2xl">{es.services.action.title}</h2>
              <p className="text-xl text-balance mb-2">
                {es.services.action.description}
              </p>
              <InternalLink href={`#${ToSlug(es.contact.title)}`} icon={faShare} text={es.services.start} />
            </div>
          </div>
        </BoxContent>
          ...
          <div className="w-full flex gap-4 items-center flex-col lg:flex-row">
            ...
            <div className="flex-1">
              <div className="flex px-8 gap-2.5 md:gap-6 items-center flex-col md:flex-row">
                <Image
                  className="rounded-full size-[100px] shadow-glow-reduced"
              ...
              <div className="bg-dark-items p-2 mt-3 text-dark-gray rounded-xl border border-dark-blue-200 flex flex-col *:w-full">
